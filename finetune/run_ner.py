...
def main():
    ...
    # Training phase.
    instances = read_bank_dataset(args, args.train_path)
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        f1 = evaluate(args, read_bank_dataset(args, args.dev_path))
        if f1 > best_f1:
            best_f1 = f1

            torch.save(model.state_dict(), args.output_model_path)

            torch.onnx.export(model, (src, src, src), "uer_py_ner_part.onnx", input_names="embedding",
                              output_names="output_layer", verbose=True)
    ...
    if args.test_path is not None:
        ...
        evaluate(args, read_bank_dataset(args, args.test_path))
